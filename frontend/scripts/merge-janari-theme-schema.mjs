/**
 * Merge platform + theme customizer schema files → janari/theme.json settings_schema.
 * Source of truth: split JSON files (not theme.json).
 *
 * Usage: node scripts/merge-janari-theme-schema.mjs
 */
import fs from 'fs';
...
console.log(
    `merged ${Object.keys(merged).length} keys → theme.json (platform ${Object.keys(omitMeta(platform)).length}, theme ${Object.keys(omitMeta(themeSchema)).length})`,
);
