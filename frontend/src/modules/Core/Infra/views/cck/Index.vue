  ...
  >
    <template #actions>
      <Button
        size="sm"
        class="h-9 gap-2"
        @click="router.push({ name: 'cck-create' })"
      >
        <Plus class="h-4 w-4" />
        {{ $t('infra.cck.newType') }}
      </Button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <ConsoleStatCard
        :label="$t('infra.cck.stats.totalTypes')"
        :value="types.length"
        :icon="Database"
        tone="primary"
      />
      <ConsoleStatCard
        :label="$t('infra.cck.stats.activeTypes')"
        :value="activeTypesCount"
        :icon="CheckCircle"
        tone="success"
      />
      <ConsoleStatCard
        :label="$t('infra.cck.stats.totalFields')"
        :value="totalFieldsCount"
        :icon="Layers"
        tone="info"
      />
      <ConsoleStatCard
        :label="$t('infra.cck.stats.apiBase')"
      ...
      <template #toolbar>
        <div class="relative w-full sm:max-w-xs shrink-0">
          ...
          <Input
            v-model="search"
            type="text"
            :placeholder="$t('infra.cck.searchPlaceholder')"
      ...
      >
        ...
        <div class="space-y-1 max-w-sm mx-auto">
          <h4 class="text-sm font-medium text-foreground">
            {{ search ? $t('infra.cck.empty') : $t('infra.cck.empty') }}
          </h4>
          <p class="text-xs text-muted-foreground">
            {{ search ? '' : $t('infra.cck.emptyHint') }}
          </p>
        </div>
        <Button
          v-if="!search"
        ...
        <Table>
          <TableHeader>
            <TableRow class="bg-muted/40 hover:bg-muted/40">
              <TableHead class="text-xs font-semibold">{{ $t('infra.cck.table.name') }}</TableHead>
              <TableHead class="text-xs font-semibold">{{ $t('infra.cck.table.slug') }}</TableHead>
              <TableHead class="text-xs font-semibold">{{ $t('infra.cck.table.fields') }}</TableHead>
              <TableHead class="text-xs font-semibold">{{ $t('infra.cck.table.status') }}</TableHead>
              <TableHead class="text-xs font-semibold text-right">{{ $t('infra.cck.table.actions') }}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            <TableRow
            ...
            >
              ...
              <TableCell class="py-3">
                <Badge variant="outline" class="text-xs font-normal bg-background gap-1">
                  ...
                  {{ $t('infra.cck.table.fieldsCount', { count: type.fields?.length ?? 0 }) }}
                </Badge>
              </TableCell>
    ...
    <ConfirmModal
      ...
      :message="$t('infra.cck.deleteTypeConfirm', { name: typeToDelete?.name ?? '' })"
...
const typeToDelete = ref<CckContentType | null>(null);
...
async function loadTypes(): Promise<void> {
    ...
    try {
        ...
        error.value = e instanceof Error ? e.message : t('infra.cck.messages.loadFailed');
    } finally {
        loading.value = false;
    }
}
...
async function downloadOpenApiSpec(type: CckContentType): Promise<void> {
    try {
        const res = await CckService.getOpenApiBySlug(type.slug);
...
function promptDeleteType(type: CckContentType): void {
...
async function executeDeleteType(): Promise<void> {
    ...
    deleting.value = true;
    try {
        await CckService.deleteType(typeToDelete.value.id);
