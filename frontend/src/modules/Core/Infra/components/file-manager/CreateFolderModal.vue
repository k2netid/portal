<template>
  ...
  >
    <DialogContent class="console-dialog-sm">
      <DialogHeader>
        <DialogTitle>{{ $t('infra.fileManager.modals.createFolder.title') }}</DialogTitle>
        <DialogDescription>
          {{ $t('infra.fileManager.modals.createFolder.placeholder') }}
        </DialogDescription>
      </DialogHeader>
      ...
      >
        <div class="grid gap-2">
          <label class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
            {{ $t('infra.fileManager.modals.createFolder.name') }} <span class="text-destructive">*</span>
          </label>
          <Input
            v-model="folderName"
            type="text"
            required
            :placeholder="$t('infra.fileManager.modals.createFolder.placeholder')"
            class="col-span-3"
          />
        </div>
      </form>
      ...
      <DialogFooter>
        <Button
          ...
          @click="$emit('close')"
        >
          {{ $t('infra.fileManager.modals.createFolder.cancel') }}
        </Button>
        <Button
        ...
        >
          ...
          />
          {{ creating ? $t('infra.fileManager.modals.createFolder.creating') : $t('infra.fileManager.modals.createFolder.create') }}
        </Button>
      </DialogFooter>
    </DialogContent>
  </Dialog>
