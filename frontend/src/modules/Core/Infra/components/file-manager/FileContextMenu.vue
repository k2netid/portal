<template>
  <ContextMenuContent class="w-56">
    ...
    >
      ...
      {{ t('infra.fileManager.actions.extract') }}
    </ContextMenuItem>

    <ContextMenuItem @click="compressItems([item.path])">
      <Archive class="w-4 h-4 mr-2" />
      {{ t('infra.fileManager.actions.compress') }}
    </ContextMenuItem>
...
const extractFile = async (file: FileItem) => {
    try {
        ...
        toast.success.action(t('infra.fileManager.messages.extracted'));
        await fetchCurrentPath();
    } catch {
        toast.error.default(t('infra.fileManager.messages.extract_failed'));
    }
};
...
const compressItems = async (paths: string[]) => {
    try {
        ...
        toast.success.action(t('infra.fileManager.messages.compressed'));
        await fetchCurrentPath();
    } catch {
        toast.error.default(t('infra.fileManager.messages.compress_failed'));
    }
};
</script>
