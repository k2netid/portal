...
- **User Creation Email Verification Controls**: Added explicit "Verifikasi Email Langsung" toggle in `UserModal.vue` allowing authorized administrators to mark accounts verified on creation ([ADR-019](../../../../docs/adr/ADR-019-user-email-verification-and-privilege-lifecycle.md)).
...
### Changed
- **Unified Role Hierarchy & Ranks**: Synchronized `ROLE_RANKS` in `modules/Core/System/stores/auth.ts` and `engine/stores/auth.ts` (`super: 100 > system-admin: 95 > admin: 90 > security-officer: 85 > operator: 80 > editor: 60 > author: 40 > staff: 30 > member: 10`) and added `hasRole` getter ([ADR-020](../../../../docs/adr/ADR-020-rbac-hierarchy-route-hardening-and-ui-primitives.md)).
...
### Fixed
- **Global 403 Forbidden Event**: Dispatched `window.dispatchEvent(new CustomEvent('app:forbidden'))` in `engine/api/client.ts` to provide responsive UI feedback on unauthorized operations ([ADR-020](../../../../docs/adr/ADR-020-rbac-hierarchy-route-hardening-and-ui-primitives.md)).
