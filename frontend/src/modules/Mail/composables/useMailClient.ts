...
export function useMailClient() {
    ...
    const mailAccountParams = (): Record<string, string> => {
        if (activeAccountId.value) {
            return { account_id: activeAccountId.value };
        }
        return {};
    };

    const withAccountId = <T extends Record<string, unknown>>(payload: T): T & { account_id?: string } => {
        if (activeAccountId.value) {
            return { ...payload, account_id: activeAccountId.value };
        }
        return payload;
    };
    ...
    const syncMailbox = async () => {
        syncing.value = true;
        try {
            const res = await api.post('/manage/mail/sync');
            await fetchMessages(1);
            const mode = res.data?.data?.mode;
            const hint = mode === 'local_index'
                ? 'Local mailbox index refreshed (full IMAP sync is not available in kernel yet)'
    ...
    const reply = (message: MailMessage, initialBody?: string) => {
        openComposer({
            to: message.sender.email,
            subject: message.subject.startsWith('Re:') ? message.subject : `Re: ${message.subject}`,
            body: initialBody ?? `<br/><br/><blockquote>On ${message.date}, ${message.sender.name} wrote:<br/>${message.snippet}</blockquote>`,
        });
    };
    ...
    const saveDraft = async (data?: { to?: string; cc?: string; bcc?: string; subject?: string; body?: string }) => {
        ...
        try {
            await api.post('/manage/mail/messages/draft', withAccountId(payload));
