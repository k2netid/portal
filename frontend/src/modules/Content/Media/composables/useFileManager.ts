...
import type { FileItem, FolderItem, TrashItem } from '@/modules/Content/Media/types/file-manager';
...
export function useFileManager(options: { rootPath?: string } = {}) {
    ...
    const deleteItem = async (item: FileItem | FolderItem) => {
        ...
        const title = isFolder ? t('media.file_manager.actions.delete_folder') : t('media.file_manager.actions.delete_file');
        const message = isFolder ? t('media.file_manager.messages.deleteFolderConfirm', { name: item.name }) : t('media.file_manager.messages.deleteFileConfirm', { name: item.name });
        ...
        try {
            ...
            fetchTrash();
            toast.success.action(isFolder ? t('media.file_manager.messages.folderDeleted') : t('media.file_manager.messages.fileDeleted'));
        } catch (error: unknown) {
    ...
    const bulkDelete = async () => {
        const confirmed = await confirmDialog({
            title: t('media.file_manager.bulk.delete'),
            message: t('media.file_manager.bulk.confirmDelete', { count: selectedItems.value.length }),
            variant: 'danger',
            confirmText: t('common.actions.delete'),
        });
        ...
        try {
            ...
            await fetchCurrentPath();
            fetchTrash();
            toast.success.action(t('media.file_manager.messages.bulkDeleted'));
        } catch (error: unknown) {
            toast.error.fromResponse(error);
        }
    };
    ...
    const copyToClipboard = (items: (FileItem | FolderItem)[], action: 'copy' | 'move' = 'copy') => {
        ...
        toast.success.action(t(`media.file_manager.messages.${action === 'copy' ? 'copiedToClipboard' : 'movedToClipboard'}`));
    };
    ...
    const pasteFromClipboard = async (destinationPath: string) => {
        ...
        try {
            ...
            await fetchCurrentPath();
            toast.success.action(t('media.file_manager.messages.pasted'));
        } catch (error: unknown) {
            toast.error.fromResponse(error);
        } finally {
            loading.value = false;
    ...
    const moveItem = async (sourcePath: string, destinationPath: string, type: 'file' | 'folder') => {
        try {
            ...
            await fetchAllFolders();
            await fetchCurrentPath();
            toast.success.action(t('media.file_manager.messages.moved'));
        } catch (error: unknown) {
            toast.error.fromResponse(error);
        }
    };
    ...
    const restoreTrashItem = async (item: TrashItem) => {
        try {
            ...
            await fetchAllFolders();
            await fetchCurrentPath();
            toast.success.action(t('media.file_manager.messages.restored'));
        } catch (error) {
            toast.error.fromResponse(error);
        }
    };
    ...
    const emptyTrash = async () => {
        const confirmed = await confirmDialog({
            title: t('media.file_manager.trash.empty'),
            message: t('media.file_manager.trash.emptyConfirm') || 'Are you sure you want to permanently delete all items in trash?',
            variant: 'danger',
            confirmText: t('media.file_manager.trash.empty'),
        });
        if (!confirmed) return;
        try {
            await api.post(infraPaths.fileManagerTrashEmpty);
            toast.success.action(t('media.file_manager.messages.trashEmptied'));
            fetchTrash();
        } catch (error) {
            toast.error.fromResponse(error);
        }
    ...
    const deleteFromTrashPermanent = async (item: TrashItem) => {
        const confirmed = await confirmDialog({
            title: t('media.file_manager.trash.permanent_delete'),
            message: t('media.file_manager.trash.confirmPermanentDelete', { name: item.name }),
            variant: 'danger',
            confirmText: t('common.actions.delete'),
        });
        if (!confirmed) return;
        try {
            ...
            toast.success.action(t('media.file_manager.messages.deleteSuccess'));
            fetchTrash();
        } catch (error) {
            toast.error.fromResponse(error);
        }
