<template>
  <div class="file-manager-container space-y-6 min-w-0 max-w-full">
        <PageHeader
      borderless
      :title="t('media.file_manager.title')"
      :subtitle="t('media.file_manager.description')"
    >
      <template #actions>
        <div class="flex items-center space-x-3">
                ...
                >
                  ...
                  {{ t('media.file_manager.actions.newFolder') }}
                </Button>
                <Button
                  type="button"
                ...
                >
                  ...
                  {{ t('media.file_manager.actions.upload') }}
                </Button>
              </div>
      </template>
    </PageHeader>
                ...
                <div class="h-full overflow-y-auto custom-scrollbar">
                  ...
                  <div v-else-if="fm.loading.value" class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-background/60 backdrop-blur-[2px]">
                    ...
                    <p class="mt-4 text-xs font-bold text-primary animate-pulse uppercase tracking-widest leading-none">
                      {{ t('media.file_manager.messages.loading') }}
                    </p>
                  </div>
                  ...
                  <div v-else-if="!fm.loading.value && (fm.paginatedFolders.value.length === 0 && fm.paginatedFiles.value.length === 0)" class="flex flex-col items-center justify-center h-full p-12 text-center">
                    ...
                    <h3 class="text-lg font-bold text-foreground/90">
                      {{ t('media.file_manager.messages.noFiles') }}
                    </h3>
                  </div>
...
import { useFileManager } from '@/modules/Content/Media/composables/useFileManager';
import FileSidebar from '@/modules/Content/Media/components/file-manager/FileSidebar.vue';
import FileToolbar from '@/modules/Content/Media/components/file-manager/FileToolbar.vue';
import FilePropertiesSidebar from '@/modules/Content/Media/components/file-manager/FilePropertiesSidebar.vue';
import FileGridView from '@/modules/Content/Media/components/file-manager/FileGridView.vue';
import FileListView from '@/modules/Content/Media/components/file-manager/FileListView.vue';
import FileTrashView from '@/modules/Content/Media/components/file-manager/FileTrashView.vue';
import FilePreviewModal from '@/modules/Content/Media/components/file-manager/FilePreviewModal.vue';
import FileUploadModal from '@/modules/Content/Media/components/file-manager/FileUploadModal.vue';
import CreateFolderModal from '@/modules/Content/Media/components/file-manager/CreateFolderModal.vue';
import type { FileItem } from '@/modules/Content/Media/types/file-manager';
