          ...
          <Tooltip>
            <TooltipTrigger as-child>
              <Button 
                ...
                :aria-label="$t('media.file_manager.actions.home')"
            ...
            <TooltipContent side="bottom">
              {{ $t('media.file_manager.actions.home') }}
            </TooltipContent>
          </Tooltip>
          <template
        ...
        <div class="flex-1 flex justify-center min-w-0">
          <div class="relative group w-full max-w-md">
            ...
            <Input
              v-model="searchQuery"
              type="text"
              :placeholder="$t('media.file_manager.actions.search')"
          ...
          <Tooltip>
            <TooltipTrigger as-child>
              <Button
                ...
                :aria-label="viewMode === 'grid' ? $t('media.file_manager.bulk.list_view') : $t('media.file_manager.bulk.grid_view')"
            ...
            <TooltipContent side="bottom">
              {{ viewMode === 'grid' ? $t('media.file_manager.bulk.list_view') : $t('media.file_manager.bulk.grid_view') }}
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
        ...
        <div class="flex items-center justify-between gap-4 overflow-x-auto flex-nowrap custom-scrollbar pb-1">
          <div class="flex items-center gap-2">
            <div class="h-8 flex-shrink-0 flex items-center px-3 rounded-lg bg-primary/10 text-primary text-xs font-semibold select-none border border-primary/20">
              {{ selectedItems.length }} <span class="hidden sm:inline ml-1">{{ $t('media.file_manager.bulk.label_count') }}</span>
            </div>
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <span>{{ $t('media.file_manager.actions.copy') }}</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent side="bottom">
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <span>{{ $t('media.file_manager.actions.move') }}</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent side="bottom">
                  {{ $t('media.file_manager.actions.move') }}
                </TooltipContent>
              </Tooltip>
              ...
              <Tooltip>
                <TooltipTrigger as-child>
                  ...
                  >
                    ...
                    <span>{{ $t('media.file_manager.actions.delete') }}</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent side="bottom">
          ...
          <Tooltip>
            <TooltipTrigger as-child>
              ...
              >
                {{ $t('media.file_manager.bulk.clear_selection') }}
          ...
          <div class="space-y-1.5 min-w-0">
            ...
            <Select v-model="filterType">
              <SelectTrigger class="bg-background h-10 border-border/40 rounded-lg w-full">
                <SelectValue :placeholder="$t('media.file_manager.filter.all')" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">
                  {{ $t('media.file_manager.filter.all') }}
                </SelectItem>
                <SelectItem value="images">
                  {{ $t('media.file_manager.filter.images') }}
                </SelectItem>
                <SelectItem value="videos">
                  {{ $t('media.file_manager.filter.videos') }}
                </SelectItem>
                <SelectItem value="documents">
                  {{ $t('media.file_manager.filter.documents') }}
                </SelectItem>
                <SelectItem value="audio">
                  {{ $t('media.file_manager.filter.audio') }}
                </SelectItem>
                <SelectItem value="archives">
                  {{ $t('media.file_manager.filter.archives') }}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div class="space-y-1.5 min-w-0">
            ...
            <div class="flex items-center gap-1.5">
              <Select v-model="sortBy">
                <SelectTrigger class="bg-background h-10 border-border/40 rounded-lg w-full">
                  <SelectValue :placeholder="$t('media.file_manager.sort.name')" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="name">
                    {{ $t('media.file_manager.sort.name') }}
                  </SelectItem>
                  <SelectItem value="size">
                    {{ $t('media.file_manager.sort.size') }}
                  </SelectItem>
                  <SelectItem value="date">
                    {{ $t('media.file_manager.sort.date') }}
                  </SelectItem>
                </SelectContent>
              </Select>
              <Button
                ...
                :aria-label="sortDirection === 'asc' ? $t('media.file_manager.sort.asc') : $t('media.file_manager.sort.desc')"
