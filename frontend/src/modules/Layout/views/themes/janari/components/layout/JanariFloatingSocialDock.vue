<template>
  <aside
    ...
    class="janari-social-dock"
    :class="[
      `janari-social-dock--${dockPosition}`,
      {
        'janari-social-dock--collapsed': isCollapsed,
        'janari-social-dock--show-mobile': showOnMobile,
      }
    ]"
  ...
  >
    ...
    >
      <button
        ref="toggleBtnRef"
        type="button"
        class="janari-social-dock__toggle-btn"
        :aria-label="t('common.socialExpand', 'Buka Media Sosial')"
        @click="handleToggle(false)"
      >
        <Share2 class="w-4 h-4 transition-transform group-hover:scale-110" />
      </button>
      <span class="janari-social-dock__tooltip">
        {{ t('common.socialMedia', 'Media Sosial') }}
      </span>
    </div>
    ...
    <div
      v-else
      ref="bodyRef"
      class="janari-social-dock__body"
    >
      <!-- Mini Header / Collapse Button -->
      <button
        type="button"
        class="janari-social-dock__collapse-btn"
        :aria-label="t('common.socialCollapse', 'Sembunyikan')"
      ...
      <div class="janari-social-dock__divider" />

      <!-- Social Items List -->
      <div class="janari-social-dock__list">
        <a
          ...
          class="janari-social-dock__item group"
...
const handleToggle = async (collapsed: boolean) => {
  ...
  if (collapsed && toggleBtnRef.value) {
    ...
    const items = bodyRef.value.querySelectorAll('.janari-social-dock__item');
    if (items.length) {
      gsap.fromTo(
        items,
        { scale: 0.7, opacity: 0 },
...
const defaultSocialLinks = [
  ...
  { icon: 'Mail', url: 'contact@jejakawan.com' },
];
