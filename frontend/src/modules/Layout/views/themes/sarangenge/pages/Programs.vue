<template>
  <SarangengePageGate
    setting-key="enable_programs"
    :title="t('pages.programs.heading', 'Kompetensi Keahlian')"
  >
...
const { data: dynamicPrograms, hasBinding } = useThemeDataBindings('programs', 'items');

const programs = ref<Content[]>([]);
const loading = ref(true);

const resolvedPrograms = computed(() => {
  if (hasBinding.value && dynamicPrograms.value && dynamicPrograms.value.length > 0) {
    return dynamicPrograms.value.map((item: any, idx: number) => {
      const raw = item._raw || item;
      return {
        id: raw.id || idx + 1,
        title: item.title || raw.title || '',
        slug: raw.slug || (item.title ? String(item.title).toLowerCase().replace(/\s+/g, '-') : `program-${idx + 1}`),
        excerpt: item.excerpt || raw.excerpt || raw.description || '',
        intro: raw.intro || item.excerpt || '',
      } as Content;
    });
  }
