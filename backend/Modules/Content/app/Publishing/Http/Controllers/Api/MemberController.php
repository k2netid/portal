...
use Modules\Content\Publishing\Models\Bookmark;
...
class MemberController extends BaseApiController
{
    ...
    public function newsletterStatus(Request $request): JsonResponse
    {
        ...
        $subscriber = NewsletterSubscriber::where('email', $user->email)
            ->withTrashed()
            ->first();
    ...
    public function updateNewsletter(Request $request): JsonResponse
    {
        ...
        if ($wantsSubscribe) {
            ...
            if ($subscriber) {
                // Reactivate if previously unsubscribed or soft-deleted
