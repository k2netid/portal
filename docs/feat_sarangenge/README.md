...
15. **[ADR-014: Presets Navigasi Samping Mengambang, GSAP Spring Motion, dan Viewport Scroll Snap Tema Sarangenge](../adr/ADR-018-theme-sarangenge-side-nav-presets-and-viewport-scroll-snap.md)**: Komponen navigasi titik melayang (`SarangengeSideNav.vue`) dengan 4 preset gaya visual (`glass`, `minimal`, `glow`, `bars`), animasi GSAP spring transition, tooltip i18n, isolasi DOM `<Teleport to="body">` di tengah vertikal viewport, dan scroll snap proporsional `yMandatory`.
16. **[ADR-015: Tata Kelola Verifikasi Email Pengguna Baru dan Proteksi Otorisasi Pembuatan Akun](../adr/ADR-019-user-email-verification-and-privilege-lifecycle.md)**: Default `is_verified = false` untuk user baru, proteksi wewenang admin untuk verifikasi instan via checkbox modal, dan pesan status penolakan login yang akurat.
17. **[ADR-016: Standarisasi Hirarki Role RBAC, Pengamanan Rute Backend, dan Primitives UI Komprehensif](../adr/ADR-020-rbac-hierarchy-route-hardening-and-ui-primitives.md)**: Penyelarasan bobot role ranks (10-100), seeder non-destruktif `givePermissionTo(...)`, pengamanan Spatie middleware pada seluruh rute backend, dan standar UI/UX (`v-can`, `v-role`, `<Can>`, custom event 403 `app:forbidden`).
