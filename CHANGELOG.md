...
### Added
- **Theme Customizer Default Dark Mode & Default Site Locale**: Added `default_theme_mode` (`dark`, `light`, `system`) under Appearance/Gaya panel and `default_site_locale` (`id`, `en`, `su`, `auto`) under General/Umum panel in global settings schema and Layung theme presets. Integrated runtime prepaint in `theme-prepaint.js`, `useDarkMode.ts`, `useTheme.ts`, and `i18n.ts` to guarantee instant dark mode and Indonesian locale rendering without white flash ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
- **Unified Services Portfolio Hub**: Consolidated `Solusi.vue` into `Services.vue` as the single canonical services hub for K2NET with interactive tab switcher (`#isp` for Internet and `#msp` for Managed Services), dynamic header, breadcrumb synchronization, and restored `<SlaGuaranteeSection />` on the ISP tab ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
- **Team Page & Navigation Integration**: Added English standard `Team.vue` page (renamed from `Tim.vue`) with SLA escalation matrix and NOC operations, integrated into public route `/team` (with alias `tim`), and integrated into main navigation menu across database seeders and sample bundles ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
- **Dynamic Chunk Import Auto-Recovery**: Added `attemptChunkRecoveryReload` in `chunkRecovery.ts` and registered it with `router.onError` in public router to gracefully recover from stale browser chunk caches during rolling deployments ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
...
- **Release & versioning guide**: Product SemVer SoT (`package.json`), `config('app.version')` / OpenAPI alignment, `npm run versions:check` in CI ([release-and-versioning.md](docs/guides/release-and-versioning.md)).
- **Modular pack SemVer discipline**: Runtime diffs under Modules/themes require manifest/`theme.json` bump + CHANGELOG (`npm run modules:versions:check`); forward-only baseline.
- **Testing / QA docs**: Suite map [`docs/reference/testing.md`](docs/reference/testing.md), Playwright how-to [`docs/guides/run-e2e-playwright.md`](docs/guides/run-e2e-playwright.md); clarify `agent:verify` vs CI PHPUnit/E2E.
...
### Changed
- **Asset Deployment & Sync Pipeline**: Hardened `sync-frontend-assets-to-backend.sh` to synchronize all HTML entrypoints (`public.html`, `landing.html`, `index.html`) and static runtime files (`theme-prepaint.js`) alongside `/assets/` directory ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
- **Content Security Policy for Cloudflare**: Added Cloudflare Turnstile / Challenge origins (`challenges.cloudflare.com`, `*.cloudflare.com`) to `SecurityHeaders.php` script-src and connect-src CSP directives ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
...
- **Product SemVer alignment**: packages / composer / README → `1.0.0-beta.2` (match CHANGELOG cut); OpenAPI + `site_version` follow `config('app.version')`.
...
### Fixed
- **Subresource Integrity (SRI) Digest Mismatch on `theme-prepaint.js`**: Introduced `strip-prepaint-sri` plugin in `vite.config.ts` and post-build safeguard in `sync-frontend-assets-to-backend.sh` to remove SRI hashes from the static unhashed `/theme-prepaint.js` resource while maintaining SRI integrity across hashed `/assets/*` bundles, preventing browser script blocking caused by edge/CDN caching ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
- **Tag Management Reactivity & Filter State**: Fixed reactivity and tab-switching state synchronization in `TagController.php` and `frontend/src/modules/Library/views/tags/Index.vue` when transitioning between general tags and editorial tags, ensuring reliable re-fetching and clean form state.
- **Legacy Route & Solusi Cleanup**: Completely removed dead `Solusi.vue` and `/solusi` routes, unifying link routing to `/services#isp` and `/services#msp` ([ADR-023](docs/adr/ADR-023-theme-customizer-defaults-services-hub-and-prepaint-sri-resilience.md)).
